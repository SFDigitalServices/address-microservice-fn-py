...
from shared_code.common import func_json_response, avs_to_eas_fields_query
...
def main(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if 'search' in params :
            ...
            if '$select' in params:
                params['$select'] = avs_to_eas_fields_query(params['$select'])
            else:
                params['$select'] = avs_to_eas_fields_query("address")
            del params['search']

        response = requests.get(
            os.getenv('AVS_API_URL'),
            headers={
